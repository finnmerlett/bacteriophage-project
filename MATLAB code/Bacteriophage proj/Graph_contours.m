function xyzData = Graph_contours(data,robust,flat)
...
xy_m = p(gxy./gxy(1), 2);
xy_c = 3.5:-1:-0.5; %initialise with y values at x = 1
xy_c = xy_c - xy_m; %convert to y intercept values (at x = 0)
ntsec = length(xy_c);

tsec_x = 1:0.1:ncol;
tsec_xs = repmat({tsec_x},ntsec,1);
tsec_ys = repmat({zeros(1,length(1:0.1:ncol))},ntsec,1);
for i = 1:ntsec %calculate y values for x values
    tsec_y = xy_m.*tsec_x + xy_c(i);
    %delete out of range values
    tsec_xs(i) = {tsec_x(1 < tsec_y & tsec_y < nrow)};
    tsec_ys(i) = {tsec_y(1 < tsec_y & tsec_y < nrow)};
end
% ------------------------------------------------------------------------
...
%------------------------------------------------------------------------
    syn_calbr = 30;   % ADJUST 'syn_calbr' VALUE HERE
%------------------------------------------------------------------------
